...
const footer = `
  <div style="margin-top:36px;padding-top:20px;border-top:1px solid #e5e7eb;text-align:center;">
    <a href="https://bridgn.com" style="text-decoration:none;"><img src="https://bridgn.com/assets/bridgn%20logos/PNG/bridgn-08.png" alt="bridgn" style="height:20px;width:auto;opacity:.5;" /></a>
    ...
    <div style="background:#f3f4f6;padding:32px 16px;">
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:540px;margin:0 auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.06);">
        <div style="background:linear-gradient(135deg,#0f4c81,#1a7fa8);padding:24px 32px;text-align:center;">
          <a href="https://bridgn.com" style="text-decoration:none;"><img src="https://bridgn.com/assets/bridgn%20logos/PNG/bridgn-18.png" alt="bridgn" style="height:28px;width:auto;" /></a>
        </div>
...
function paymentSettledEmail({ brandName, creatorName, amount, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
    ...
    html: wrap(`
      ...
      <a href="${link}" style="display:inline-block;padding:10px 24px;background:#5b8af5;color:white;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;">
        Open Deal Room
      </a>
    `),
  };
...
function contentSubmittedEmail({ brandName, creatorName, fileName, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function scriptSharedEmail({ brandName, creatorName, scriptUrl, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function briefSubmittedEmail({ creatorName, brandName, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function contractSentEmail({ creatorName, brandName, dealTitle, hasLink, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function contractSignedEmail({ brandName, creatorName, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function newMessageEmail({ recipientName, senderName, messagePreview, dealTitle, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function dealInviteEmail({ recipientName, senderName, senderRole, dealTitle, amount, platform, deliverables, deadline, inviteLink }) {
  ...
  const platformIcons = {Instagram:"IG",TikTok:"TT",YouTube:"YT",Twitter:"X",Multiple:"—"};
  return {
...
function contentLiveEmail({ brandName, creatorName, dealTitle, postLink, remainingAmount, dueDate, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function paymentOverdueEmail({ brandName, dealTitle, amount, daysOverdue, dealId }) {
  const link = dealId ? `https://bridgn.com?page=deal-room&deal=${dealId}` : "https://bridgn.com?page=deal-room";
  return {
...
function deadlineReminderEmail({ recipientName, dealTitle, deadline, daysLeft, partnerName, dealId }) {
...
// ── Guest deal notification — email relay for solo creator mode ──────────────

function guestDealNotificationEmail({ brandContactName, creatorName, dealTitle, contentType, approveLink, changesLink, viewLink }) {
  const typeLabel = contentType === "script" ? "a script" : contentType === "brief" ? "a creative brief" : "content";
  return {
    subject: `${creatorName} shared ${typeLabel} for "${dealTitle}"`,
    html: wrap(`
      <p style="font-size:15px;font-weight:600;margin:0 0 8px;">Hi${brandContactName ? " " + brandContactName : ""},</p>
      <p style="font-size:14px;color:#4b5563;line-height:1.6;margin:0 0 16px;">
        <strong>${creatorName}</strong> shared ${typeLabel} for <strong>${dealTitle}</strong> and is waiting for your review.
      </p>
      <div style="margin:20px 0;display:flex;gap:12px;">
        <a href="${approveLink}" style="display:inline-block;padding:12px 28px;background:#059669;color:white;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;">
          Approve
        </a>
        <a href="${changesLink}" style="display:inline-block;padding:12px 28px;background:#f59e0b;color:white;text-decoration:none;border-radius:8px;font-weight:600;font-size:14px;">
          Request Changes
        </a>
      </div>
      <p style="font-size:13px;color:#9ca3af;margin:16px 0 0;">
        Want to collaborate in real time? <a href="${viewLink}" style="color:#5b8af5;">Join the Deal Room</a> to message, review content, and manage this deal together.
      </p>
    `),
  };
}
