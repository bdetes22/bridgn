...
/**
 * Upsert a deal row. Tries to find the existing deal by bridgn_deal_id first
 * (since that's the unique key), then falls back to payment_intent_id.
 * If neither exists, creates a new row.
 */
async function upsertDeal(paymentIntentId, fields) {
  // Try to find existing deal by bridgn_deal_id first
  if (fields.bridgn_deal_id) {
    const existing = await getDealByBridgnDealId(fields.bridgn_deal_id);
    if (existing) {
      const { data, error } = await db
        .from("deals")
        .update({ payment_intent_id: paymentIntentId, ...fields })
        .eq("id", existing.id)
        .select()
        .single();
      if (error) throw error;
  ...
  // Fall back to upsert on payment_intent_id (for webhook events
  // where we only have the PI ID, not the bridgn_deal_id yet)
  const { data, error } = await db
    .from("deals")
    .upsert(
...
async function getSubscriptionStatus(userId, role) {
  ...
  const { data, error } = await db
    ...
    .maybeSingle();
  if (error) throw error;
  return data || { subscription_status: "none", trial_ends_at: null, stripe_subscription_id: null };
}

async function updateSubscription(userId, role, fields) {
  const table = role === "brand" ? "brand_profiles" : "creator_profiles";
  const { data, error } = await db
    .from(table)
    .upsert({ user_id: userId, ...fields }, { onConflict: "user_id" })
...
async function getDealByGuestToken(token) {
  const { data, error } = await db
    .from("deals")
    .select("*")
...
async function countActiveDealsForUser(userId) {
  const { count, error } = await db
    .from("deals")
    .select("id", { count: "exact", head: true })
    .or(`creator_user_id.eq.${userId},brand_user_id.eq.${userId}`)
    .not("status", "in", '("Completed","Expired","payment_released","Cancelled","Declined")');
  if (error) throw error;
  return count || 0;
}
